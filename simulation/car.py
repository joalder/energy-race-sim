...
from simulation.environment import Position, Environment
...
class Car(Tickable):
    def __init__(self, max_acceleration: float, max_speed, energy_stored=0, energy_used=0, position=Position(),
                 current_speed=0, distance_driven=0):
    ...
    def apply(self, environment: Environment, time_delta: int) -> Self:
        ...
        log.info(self.status_delta(time_delta, energy_delta, speed_delta, acceleration, distance_delta))
        derived_car = self.derive_car(energy_delta, speed_delta, distance_delta, new_position)
        return derived_car
    ...
    def derive_car(self, energy_delta, speed_delta, distance_delta, new_position):
        return Car(
